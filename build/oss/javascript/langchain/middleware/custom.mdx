...
You can modify system prompts in middleware using either `systemPrompt` (`string`) or `systemMessage` ([`SystemMessage`](https://reference.langchain.com/javascript/classes/_langchain_core.messages.SystemMessage.html)). The [`ModelRequest`](https://reference.langchain.com/javascript/classes/_langchain_core.messages.ModelRequest.html) provides both for maximum flexibility.

**Key behavior:**
- Middleware receives both `systemPrompt` (string) and `systemMessage` ([`SystemMessage`](https://reference.langchain.com/javascript/classes/_langchain_core.messages.SystemMessage.html)) in the request
